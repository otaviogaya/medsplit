...
begin
  if not exists (select 1 from pg_type where typname = 'forma_pagamento_tipo') then
    create type forma_pagamento_tipo as enum ('dinheiro', 'pix', 'cartao');
  end if;
end $$;

alter table public.procedimentos
  add column if not exists forma_pagamento forma_pagamento_tipo;

drop function if exists public.update_status_procedimento(uuid, procedimento_status, date, date, numeric, text);
