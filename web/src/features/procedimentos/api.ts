...
export async function createProcedimento(payload: ProcedimentoCreatePayload) {
  const { data, error } = await supabase.rpc("create_procedimento", { payload });
  if (error) throw error;
  return data;
}

export async function uploadProcedimentoDocumento(file: File) {
...
export async function updatePagamentoProcedimento(id: string, pagamentoStatus: "nao_pago" | "pago") {
